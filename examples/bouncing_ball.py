...
# Define the bouncing ball system using a PyTorch module
class BouncingBallExample(nn.Module):
    def __init__(self, radius=0.2, gravity=9.8, adjoint=False):
        super().__init__()
        self.gravity = nn.Parameter(torch.as_tensor([gravity]))  # Gravity parameter
        self.log_radius = nn.Parameter(torch.log(torch.as_tensor([radius])))  # Log of the ball's radius
        self.t0 = nn.Parameter(torch.tensor([0.0]))  # Initial time
        self.init_pos = nn.Parameter(torch.tensor([10.0]))  # Initial position
        self.init_vel = nn.Parameter(torch.tensor([0.0]))  # Initial velocity
        self.absorption = nn.Parameter(torch.tensor([0.2]))  # Energy absorption factor on bounce
        self.odeint = odeint_adjoint if adjoint else odeint  # Choose ODE integrator

    def forward(self, t, state):
        pos, vel, log_radius = state
        dpos = vel  # Derivative of position is velocity
        dvel = -self.gravity  # Acceleration due to gravity
        return dpos, dvel, torch.zeros_like(log_radius)  # Log radius remains constant

    def event_fn(self, t, state):
        # positive if ball in mid-air, negative if ball within ground.
        pos, _, log_radius = state
        return pos - torch.exp(log_radius) # Event function to detect collision with the ground
    ...
    def state_update(self, state):
        ...
        pos = (
            pos + 1e-7
        )  # Small adjustment to position to avoid immediate retriggering of event.
        vel = -vel * (1 - self.absorption)  # Invert velocity and apply absorption.
...
# Function to check gradients for debug
