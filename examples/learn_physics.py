...
# Define a class for modeling Hamiltonian dynamics
class HamiltonianDynamics(nn.Module):
    def __init__(self):
        super().__init__()
        self.dvel = nn.Linear(1, 1)  # Linear layer for velocity change
        self.scale = nn.Parameter(torch.tensor(10.0))   # Scale factor for dynamics

    def forward(self, t, state):
        pos, vel, *rest = state  # Unpack position and velocity
        dpos = vel  # Position change is velocity
        dvel = torch.tanh(self.dvel(torch.zeros_like(vel))) * self.scale  # Apply non-linearity to velocity change
        return (dpos, dvel, *[torch.zeros_like(r) for r in rest])  # Return new state

# Event function class for determining physics events like collisions
class EventFn(nn.Module):
    def __init__(self):
        super().__init__()
        self.radius = nn.Parameter(torch.rand(1)) # Radius parameter for events
    ...
    def forward(self, t, state):
        ...
        pos, _, radius = state  # Check position against radius to determine event
        return pos - radius.reshape_as(pos) ** 2

# Module to change state instantaneously (e.g., after collisions)
class InstantaneousStateChange(nn.Module):
    def __init__(self):
        super().__init__()
        self.net = nn.Linear(1, 1)  # Neural net for state change

    def forward(self, t, state):
        pos, vel, *rest = state
        vel = -torch.sigmoid(self.net(torch.ones_like(vel))) * vel  # Apply change to velocity
        return (pos, vel, *rest)

# Neural network model for simulating physics with neural networks
class NeuralPhysics(nn.Module):
    def __init__(self):
        super().__init__()
        self.initial_pos = nn.Parameter(torch.tensor([10.0]))  # Initial position
        self.initial_vel = nn.Parameter(torch.tensor([0.0]))  # Initial velocity
        self.dynamics_fn = HamiltonianDynamics()  # Dynamics function
        self.event_fn = EventFn()  # Event function
        self.inst_update = InstantaneousStateChange()  # State updater

    def simulate(self, times):

        t0 = torch.tensor([0.0]).to(times)

         # Define event function with terminal condition
        ...
        while t0 < times[-1] and n_events < max_events:
            ...
            traj_ = solution_[0][1:]  # [0] for position; [1:] to remove intial state. Extract position post-initial state
            trajectory.append(traj_)

            if event_t < times[-1]:
                state = tuple(s[-1] for s in solution)  # Update state post-event

                # update velocity instantaneously.
                state = self.inst_update(event_t, state) # Apply instantaneous update

                # advance the position a little bit to avoid re-triggering the event fn.
                pos, *rest = state
                pos = pos + 1e-7 * self.dynamics_fn(event_t, state)[0]  # Nudge position to avoid repeated events
                state = pos, *rest
